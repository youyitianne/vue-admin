<template>
  <div class="app-container">
    <div class="filter-container" style="margin: 15px;margin-top: -5px">
      ...
      <el-button @click="serchTable">搜索</el-button>
      <el-checkbox v-model="checked1" border style="margin-left: 15px" @change="getDatawithParam">显示删除状态SDK模版</el-checkbox>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="60%" :close-on-click-modal=false>
      <el-form ref="dataForm" :model="sdk" label-position="left" label-width="100px" :inline="true"
        ...
        <el-form-item label="SDK名称" class="filter-item" :rules="[{ required: true, message: '标记不能为空'}]" prop="mark"
        ...
        <el-form-item v-for="(domain, index) in sdk.form.domains" :key="domain.key+index">
          <div>
            ...
            <el-input v-model="domain.sdk_paramter" style="width: 300px;margin-right: 5px" type="textarea"
                      maxlength="255"
                      placeholder="必填"/>
...
<script>
  ...
  import {createSdk, getSdk, updateSdk, getChannel,getSdkLimitMeth} from '@/api/table/sdkmanager/sdklisttable'
    ...
    methods: {
      ...
      pageChange(page){
        ...
        let param={
          ...
          sdkName:this.secondary_sdk_name,
          sdkMark:this.secondary_sdk_mark,
        }
        getSdkLimitMeth(param).then(response=>{
      ...
      getDatawithParam() {
        ...
        if (this.checked1 === false) {
          for (let i = 0; i < this.hidlist.length; i++) {
            if (this.hidlist[i].sdk_status === '1') {
